...
def graficar_deteccion_preliminar(E, I, elementos_detectados, bkg_snip=None, config=None):
    ...
    for symb in elementos_detectados:
        try:
            ...
            info_lineas = core.get_Xray_info(symb, families=("K", "L", "M"), config_anode=config.anode)
...
def graficar_ajuste(E, I, I_fit, bkg_fit, elementos, popt, p=None, shells=["K", "L", "M"], 
    ...
    if shells != ["scat"]:
        for elem in elementos:
            ...
            try:
                ...
                info = core.get_Xray_info(elem, families=tuple(shells), config_anode=config.anode)
            except:
                continue
    ...
    if config:
        ...
        tube_info = core.get_Xray_info(config.anode, families=("K", "L"), config_anode=config.anode)
...
def generar_reporte_completo(E, I, I_fit, bkg_fit, popt, elementos, nombre_muestra="Muestra", n_top=3, 
    ...
    if n_plots > 0:
        ...
        for i, res in enumerate(peores):
            ...
            info = core.get_Xray_info(res['elemento'], config_anode=config.anode)
...
def exportar_reporte_pdf(E, I, I_fit, bkg_fit, popt, elementos, config, nombre_muestra="Muestra", 
    ...
    with PdfPages(archivo) as pdf:
        ...
        final_params = core.pack_params(popt, elementos, n_bkg=n_bkg)
